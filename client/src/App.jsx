...
const EnhancedCalendar = ({ tasks, onSelectDate, selectedDate, onClose, onNotification }) => {
  ...
  const handleDateClick = (date) => {
    ...
    if (tasksOnDate.length > 0) {
      ...
      onNotification(`📅 No hay tareas agendadas para el ${formatDate(dateStr)}`, 'info')
    }
  }
...
const exportToExcel = (tasks, lists) => {
  import('xlsx').then((XLSX) => {
    const workbook = XLSX.utils.book_new()

    const headers = [
      { t: 's', v: 'Nombre de Tarea', s: { font: { bold: true, color: { rgb: 'FFFFFF' } }, fill: { fgColor: { rgb: '2C3E50' } }, alignment: { horizontal: 'center' } } },
      { t: 's', v: 'Lista', s: { font: { bold: true, color: { rgb: 'FFFFFF' } }, fill: { fgColor: { rgb: '2C3E50' } }, alignment: { horizontal: 'center' } } },
      { t: 's', v: 'Prioridad', s: { font: { bold: true, color: { rgb: 'FFFFFF' } }, fill: { fgColor: { rgb: '2C3E50' } }, alignment: { horizontal: 'center' } } },
      { t: 's', v: 'Fecha', s: { font: { bold: true, color: { rgb: 'FFFFFF' } }, fill: { fgColor: { rgb: '2C3E50' } }, alignment: { horizontal: 'center' } } },
      { t: 's', v: 'Hora Inicio', s: { font: { bold: true, color: { rgb: 'FFFFFF' } }, fill: { fgColor: { rgb: '2C3E50' } }, alignment: { horizontal: 'center' } } },
      { t: 's', v: 'Hora Fin', s: { font: { bold: true, color: { rgb: 'FFFFFF' } }, fill: { fgColor: { rgb: '2C3E50' } }, alignment: { horizontal: 'center' } } },
      { t: 's', v: 'Estado', s: { font: { bold: true, color: { rgb: 'FFFFFF' } }, fill: { fgColor: { rgb: '2C3E50' } }, alignment: { horizontal: 'center' } } }
    ]

    const taskData = tasks.map(task => {
      const listName = lists.find(l => l.id === task.list_id)?.name || 'Sin lista'
      const priorityText = task.priority === 'alta' ? 'Alta' : task.priority === 'media' ? 'Media' : 'Baja'
      const dateFormatted = task.date ? task.date.split('-').reverse().join('/') : 'Sin fecha'
      const startTime = task.startTime || 'Sin hora'
      const endTime = task.endTime || 'Sin hora'
      const taskDateTime = task.date ? new Date(task.date + 'T' + (task.endTime || '23:59')) : null
      const status = task.completed ? 'Completada' : (taskDateTime && taskDateTime < new Date()) ? 'Vencida' : 'Pendiente'

      const getPriorityColor = (priority) => {
        switch (priority) {
          case 'alta': return { font: { color: { rgb: 'FFFFFF' } }, fill: { fgColor: { rgb: 'E74C3C' } } }
          case 'media': return { font: { color: { rgb: '000000' } }, fill: { fgColor: { rgb: 'F1C40F' } } }
          case 'baja': return { font: { color: { rgb: 'FFFFFF' } }, fill: { fgColor: { rgb: '27AE60' } } }
          default: return {}
        }
      }
      ...
      const getStatusColor = (taskStatus) => {
        switch (taskStatus) {
          case 'Completada': return { font: { color: { rgb: 'FFFFFF' } }, fill: { fgColor: { rgb: '27AE60' } } }
          case 'Vencida': return { font: { color: { rgb: 'FFFFFF' } }, fill: { fgColor: { rgb: 'E74C3C' } } }
          case 'Pendiente': return { font: { color: { rgb: '000000' } }, fill: { fgColor: { rgb: 'BDC3C7' } } }
          default: return {}
        }
      }
      ...
      return [
        { t: 's', v: task.title },
        { t: 's', v: listName },
        { t: 's', v: priorityText, s: { alignment: { horizontal: 'center' }, ...priorityStyle } },
        { t: 's', v: dateFormatted, s: { alignment: { horizontal: 'center' } } },
        { t: 's', v: startTime, s: { alignment: { horizontal: 'center' } } },
        { t: 's', v: endTime, s: { alignment: { horizontal: 'center' } } },
        { t: 's', v: status, s: { alignment: { horizontal: 'center' }, ...statusStyle } }
      ]
    })

    const allData = [headers, ...taskData]
    ...
    XLSX.writeFile(workbook, `reporte_tareas_${date}.xlsx`)
  })
}

// ================= COMPONENTE REPORTE =================

const EnhancedReportModal = ({ tasks, lists, onClose }) => {
    ...
    <div className="modal-overlay active" onClick={onClose}>
      <div className="modal report-modal enhanced" onClick={e => e.stopPropagation()}>
        ...
        <div className="report-actions">
          <h4 className="report-actions-title">📋 Reporte de Tareas</h4>
          <button className="export-excel-btn" onClick={() => exportToExcel(tasks, lists)}>
            📥 Exportar a Excel
          </button>
        </div>
  ...
  return (
    <>
      ...
      <main className="contenido-principal">
          ...
          <h1>
            ...
            {selectedDate && (
              <span className="date-filter-badge">
                📅 {formatDate(getLocalDateString(selectedDate))}
              </span>
            )}
            {selectedDate && (
              <button 
          ...
          <div className="filtros">
            ...
            {selectedDate && (
              <button className="filtro-btn clear-date-btn" onClick={clearDateFilter}>
                ✖️ Limpiar fecha: {formatDate(getLocalDateString(selectedDate))}
              </button>
            )}
          </div>
        ...
        {showCalendar && (
          <EnhancedCalendar tasks={tasks} onSelectDate={handleDateSelect} selectedDate={selectedDate} onClose={() => setShowCalendar(false)} onNotification={showNotification} />
        )}
...
<div className={`modal-overlay delete-modal-overlay ${deleteModalTask ? 'active' : ''}`} onClick={() => setDeleteModalTask(null)}>
  ...
  {showReport && <EnhancedReportModal tasks={tasks} lists={lists} onClose={() => setShowReport(false)} />}
